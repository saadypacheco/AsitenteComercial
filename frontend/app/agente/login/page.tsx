...
export default function AgenteLoginPage() {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#e9edf3] px-4">
      <div className="w-full max-w-sm overflow-hidden rounded-3xl bg-white shadow-card-lg">
        <div className="bg-gradient-to-br from-brand to-brand-2 px-6 py-8 text-center text-white">
          ...
          {!sent ? (
            <form onSubmit={submit} className="space-y-3">
              ...
              <button type="submit" disabled={loading}
                className="w-full rounded-lg bg-brand py-2.5 text-sm font-semibold text-white shadow-card disabled:opacity-50">
            ...
            <div className="space-y-3">
              ...
              {link && (
                <div className="rounded-lg border border-line bg-white p-3 text-xs text-muted">
                  ...
                  <a href={link} className="inline-block rounded-lg bg-brand px-3 py-2 text-xs font-semibold text-white">{t.openLink}</a>
                </div>
              )}
            </div>
          )}
